...
public class YejiController {
	...
	CategoryDataset dataset = null;   //柱状数据集
	DefaultPieDataset dfp = new DefaultPieDataset();   //饼状数据集
@Resource
private ICarchartService CarchartService;

public ICarchartService getCarchartService() {
	return CarchartService;
}
public void setCarchartService(ICarchartService carchartService) {
		...
		else
		{
		 ...
		 {  double b=((double)map.get(s).getC_count()/(double)count)*100;
			 dfp.setValue(map.get(s).getC_selltime()+s+"比例:"+df.format(b)+"%", map.get(s).getC_count());  
				...
				if(year>0&month>0&day==0)
				{
					 List<Carchart> list= CarchartService.chartinfo(year, month);
					 for(Carchart s:list){
							count+=s.getC_count();
						}
					 for(Carchart c:list)
				...
				if(year==0&month==0&day==0)
				{ List<Carchart> list=  CarchartService.chartinfo();
				for(Carchart s:list){
					count+=s.getC_count();
				}
				for(Carchart s:list)
				...
				if(year>0&month==0&day==0)
				{
					Map<String, Carchart> map=  CarchartService.chartinfo(year);
					for(String s:map.keySet())
					{
	              count+=map.get(s).getC_count();
					}
