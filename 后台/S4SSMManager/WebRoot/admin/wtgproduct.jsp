...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>未通过审核的商品</h2>
		<div class="manage">
		<form action="/S4SSMManager/MgCar_showwtgwhere.action" method="post">
		 ...
		 <select  name="sel2" id="sel2" onchange="getcx(this)" >
			...
			<table class="list">
				...
				<td>${carMap.uId }</td>
					...
					<td class="thumb" ><a target="_blank" href="/S4SSM/CarInfo_showshdea.action?cid=${carMap.cId }&uid=${carMap.uId}"><img src="/S4SSM/${carMap.cImg}" /></a><a target="_blank" href="/S4SSM/CarInfo_showshdea.action?cid=${carMap.cId }&uid=${carMap.uId}">${carMap.cBrand } ${carMap.cSeries } ${carMap.cId }${carMap.cType }${carMap.cModel }</a></td>
					<td>${carMap.cState }</td>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSMManager/MgCar_showwtgwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSMManager/MgCar_showwtgwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSMManager/MgCar_showwtgwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSMManager/MgCar_showwtgwhere.action?cpp=${pp }&ccx=${cx}&jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
