		...
		<!--<form method="get">
			...
			<td class="w1 c"><input type="button" value="通过" onclick="pass(${sm.value.cId},${sm.value.cUid },${sm.value.uId })"/><input type="button" value="拒绝" onclick="nopass(${sm.value.cId},${sm.value.cUid },${sm.value.uId })"/></td>
			 ...
			 <div class="pages" align="center"> <c:if test="${curPage > 1}">
					<a href="/S4SSMManager/Mgord_showallord.action?jumpPage=1">首页</a>&nbsp;&nbsp;&nbsp;
		 		<a href="/S4SSMManager/Mgord_showallord.action?jumpPage=${curPage - 1}&">上一页</a>
				</c:if> <c:if test="${curPage <= 1}">
			 		首页&nbsp;&nbsp;&nbsp;上一页
		 		</c:if> &nbsp;&nbsp;&nbsp; <c:if test="${curPage < maxPage}">
					<a href="/S4SSMManager/Mgord_showallord.action?jumpPage=${curPage + 1}">下一页</a>
		 			&nbsp;&nbsp;&nbsp;
		 			<a href="/S4SSMManager/Mgord_showallord.action?jumpPage=${maxPage}">尾页</a>
				</c:if> <c:if test="${curPage >= maxPage}">
					下一页
		 			&nbsp;&nbsp;&nbsp;尾页		 		
		 		</c:if>
